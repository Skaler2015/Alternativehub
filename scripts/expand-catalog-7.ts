/**
 * Catalog expansion (batch 7) — 1,000+ net-new software and web apps.
 *
...
import { ROWS as RAW_ROWS } from "./catalog-7-data";
...
async function run() {
  try {
    ...
    await bulkInsert(prisma, rows, "expand-7");
  } catch (err) {
    console.warn("[expand-7] Expansion failed (deploy continues).", err);
  } finally {
